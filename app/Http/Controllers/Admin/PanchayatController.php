...
{
    protected $page = 'panchayat';

    public function __construct(Request $request)
    {
        $this->model = new States();
        $this->sortableColumns = ['districts.id', 'country.name','states.name','districts.name','tehsils.name','panchayat.name', 'districts.status', 'districts.updated_at' ];
    ...
    {
        $data = Panchayat::select('panchayat.*')
                            ->where('panchayat.id', $id)
                            ->first();
      ...
      //print_r($data);dd();
       ...
       //// print_r($tehsil_data);dd();
        if(empty($id)){ $type='Add '; }else{ $type='Update '; }
        $result = ['title' => ucwords($type .'Panchayat' ), 'page' => $this->page, 'data' => $data,'country'=>$country_data,'state'=>$state_data,'district'=>$district_data,'tehsil'=>$tehsil_data];
    ...
    public function manage(Request $request)
    {
        ...
        if(isset($id) && !empty($id)){
            $data = Panchayat::where('id', $request->id)->first();
            saveAdminNotification("Update Panchayat","Panchayat ".$request->get('panchayat_name')." updated by ");
        }else{
            
            $data = new Panchayat();
            saveAdminNotification("New Panchayat","New Panchayat ".$request->get('panchayat_name')." added by ");
            $data->status = '1';
        }

        $data->name = $request->get('panchayat_name'); 
        ...
        if ($data->save()) {
                return response()->json(['status' => true, 'message' => 'Panchayat has been '.$type.' succesfully']);
        } else {
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $response->whereBetween('panchayat.created_at', [$start_date, $end_date]);

            $response = $response->offset($start)->limit($limit)->orderBy('panchayat.name', 'ASC')->get();
            if (!$response) {
                $data = [];
                $paging = [];
            } else {
        ...
        $data = ['title' => ucfirst('Panchayat Management'), 'page' => $this->page];
    ...
    {
        $q = Panchayat::select('panchayat.*','tehsils.name as tname','districts.name as dname','states.name as sname','country.name as cname')
                    ->where('panchayat.deleted_at','=',null)
        ...
        $orderby = $orderby ? $orderby : 'panchayat.name';
        $order = $order ? $order : 'ASC';

        if ($search && !empty($search)) {
            $q->where(function ($query) use ($search) {
                $query->where('panchayat.name', 'LIKE', '%' . $search . '%')
    ...
    {
        $data = Panchayat::select('panchayat.*','states.name as sname','country.name as cname','tehsils.name as tname','districts.name as dname')
                        ->where('panchayat.id', $id)
                        ->leftJoin('districts', 'districts.id', '=', 'panchayat.district_id')
                        ->leftJoin('states', 'states.id', '=', 'panchayat.state_id')
                        ->leftJoin('country', 'country.id', '=', 'panchayat.country_id')
                        ->leftJoin('tehsils', 'tehsils.id', '=', 'panchayat.tehsil_id')
                        ->first();
       
        if (isset($data)) {
            $result = ['title' => ucwords('Panchayat Details'), 'page' => $this->page, 'data' => $data];
    ...
    {     
        ...
        if ($data->save()) {
            return response()->json(['status' => true, 'message' => 'Panchayat has been '.$status.' succesfully']);
        } else {
            return response()->json(['status' => false, 'message' => 'Something went wrong']);
        }
    }
    ...
    {     
        ...
        if ($data->save()) {
            return response()->json(['status' => true, 'message' => 'Panchayat has been deleted succesfully']);
        } else {
            return response()->json(['status' => false, 'message' => 'Something went wrong']);
        }
    }
