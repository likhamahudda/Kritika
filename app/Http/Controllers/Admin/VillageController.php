...
class VillageController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->sortableColumns = ['village.id', 'village.name','panchayat.name','tehsils.name','districts.name','states.name' ,'country.name','status', 'updated_at' ];
    ...
    public function manage(Request $request)
    {
        ...
        if(isset($id) && !empty($id)){
            ...
            saveAdminNotification("Update Village","Village ".$request->get('village_name')." updated by ");
        }else{
            
            $data = new Village();
            saveAdminNotification("New Village","New Village ".$request->get('village_name')." added by ");
            $data->status = '1';
        }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $response = $response->offset($start)->limit($limit)->orderBy('districts.name', 'desc')->get();
            if (!$response) {
                $data = [];
                $paging = [];
            } else {
    ...
    public function getData($search = null  ,$orderby = null, $order = null, $request = null)
    {
        $q = Village::select('village.*','tehsils.name as tname','districts.name as dname','panchayat.name as pname','states.name as sname','country.name as cname')
                    ...
                    ->leftJoin('districts', 'districts.id', '=', 'village.district_id')
                    ->leftJoin('tehsils', 'tehsils.id', '=', 'village.tehsil_id')
                    ->leftJoin('states', 'states.id', '=', 'village.state_id')
                    ->leftJoin('country', 'country.id', '=', 'village.country_id');
    ...
    {
        $data = Village::select('village.*','states.name as sname','country.name as cname','tehsils.name as tname','districts.name as dname','panchayat.name as pname')
                         ->where('village.id', $id)
                         ->leftJoin('districts', 'districts.id', '=', 'village.district_id')
                         ->leftJoin('states', 'states.id', '=', 'village.state_id')
                         ->leftJoin('country', 'country.id', '=', 'village.country_id')
        ...
        if (isset($data)) {
            $result = ['title' => ucwords('Village Details'), 'page' => $this->page, 'data' => $data];
...
/* Function For get state */
    ...
    { 
        $states = States::where('country_id', $request->country_id)->where('status', '1')->where('deleted_at', '=', null)->orderBy('name')->get();
...
/* Function For get Districts */
    ...
    { 
        $tehsils = Tehsils::where('district_id', $request->district_id)->where('status', '1')->where('deleted_at', '=', null)->orderBy('name')->get();
...
{ 
    $panchayat = Panchayat::where('tehsil_id', $request->tehsil_id)->where('status', '1')->where('deleted_at', '=', null)->orderBy('name')->get();
