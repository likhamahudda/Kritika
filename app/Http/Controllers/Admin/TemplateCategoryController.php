...
{
    protected $page = 'templateCategory';

    public function __construct(Request $request)
    {
        $this->model = new TemplateCategory();
        $this->sortableColumns = ['id', 'name', 'status', 'updated_at' ];
    ...
    {
        $data = TemplateCategory::where('id', $id)->first();
        if(empty($id)){ $type='Add '; }else{ $type='Update '; }
        $result = ['title' => ucwords($type .'Education' ), 'page' => $this->page, 'data' => $data];
    ...
    public function manage(Request $request)
    {
        ...
        if(isset($id) && !empty($id)){  // add new data
            if(TemplateCategory::where('name', '=', $request->get('name'))->where('id','!=', $request->id)->where('deleted_at','=',null)->count() > 0){
                return response()->json(['status' => false,  'message' => 'Education name is already exist']);
            }
            $data = TemplateCategory::where('id', $request->id)->first();
        }else{  // update data
            if(TemplateCategory::where('name', '=', $request->get('name'))->where('deleted_at','=',null)->count() > 0){
                return response()->json(['status' => false,  'message' => 'Education name is already exist']);
            }
        ...
        if ($data->save()) {
                return response()->json(['status' => true, 'message' => 'Education has been '.$type.' succesfully']);
        } else {
            return response()->json(['status' => false,  'message' => 'Something went wrong']);
        }
    }

    /* Function for view list of Education Category */
    ...
    public function index(Request $request)
    {
        ...
        $data = ['title' => ucfirst('Education'), 'page' => $this->page];
    ...
    {
        $q = TemplateCategory::where('deleted_at','=',null);
    ...
    /* Function for view Template Category deatils */

    // public function view($id)
    // {
    //     $data = TemplateCategory::where('id', $id)->first();
    //     if (isset($data)) {
    //         $result = ['title' => ucwords('Template Category Details'), 'page' => $this->page, 'data' => $data];
    ...
    {     
        ...
        if ($data->save()) {
            return response()->json(['status' => true, 'message' => 'Education has been '.$status.' succesfully']);
        } else {
            return response()->json(['status' => false, 'message' => 'Something went wrong']);
        }
    }
    ...
    {     
        ...
        if ($data->save()) {
            return response()->json(['status' => true, 'message' => 'Education has been deleted succesfully']);
        } else {
            return response()->json(['status' => false, 'message' => 'Something went wrong']);
        }
    }
